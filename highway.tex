...
\begin{itemize}
    ...
    % Validator receives a message from another validator if and only if the latter sent it and intended it to be received by the former. 
...
% \begin{itemize}
%     \item Protocols providing different guarantees based on type of the network (sync/part. sync) - XFT \cite{LVCQV16} - 
%     \item $n>3f+2c+1$ - \cite{GAGMPRSTT19} - no flexibility
% \end{itemize}
...
% For a given protocol state $\sigma$ we denote the set of proven equivocators as $$E(\sigma) = \{v\in V| \text{There exists } u,u' \text{ created by } v \text{ s.t. } u\not\leq u' \text{and } u'\not\leq u\}$$
...
% The Highway protocol applies the ''virtual voting'' principle \cite{}, meaning that a given unit $u$ does not need to explicitly carry a ''vote'' for the selected block, but rather such a vote is computed locally by each validator based only on $u$ justifications.

% Each unit $u$ votes not only for one block, but rather for a whole branch starting at genesis and ending in one of the maximal blocks in $\bar{D}(u)$.
% Highway adopts the Greedy Heaviest-Observed Sub-Tree rule of voting, or a GHOST rule, for short.
% It means that the branch voted by each unit is chosen recursively starting from the genesis block $G$, where at each step the candidate that has the most votes of the most recent units is chosen.
% More formally, we recursively define the function $\vote_k$, corresponding to the vote on $k$-th block in the chain, as follows:

% \begin{itemize}
%     \item $\vote_0(u) = G$
%     \item $\vote_k(u) = \min \left(\argmax\limits_{B\in\mathrm{next}(\vote_{k-1}(u))} \ww(\{v \in L (\bar{D}(u)) \mid \vote_{k}(v) = B\})\right)\textrm{,}$
% \end{itemize}


\subsection{Finality condition}\label{sec:finality}
% \damian{Would be good to write some prose here.}
...
% with quorum size $q$ and height $k$ for a block $B$, or a \emph{$(q,k)$-summit} for a block $B$, is a sequence of sets of units $C_0 \supseteq C_1 \supseteq C_2 \supseteq \dots \supseteq C_k$ satisfying the following properties:
\begin{itemize}
...
% As $\vote(u)\not\geq B$, it must be that $\#\{v\in L(u) \}$
...
% \begin{equation}\label{EquationSummit2}
%     \#\Big( E\big(\bar{D}(C_0\cup \{u'\})\big)\cap S(C_0)\Big) \geq (2q-n)(1-2^{-k+1})
% \end{equation}

Next, connecting Inequalities \ref{EquationSummit1} and \ref{EquationSummit2}, we get

\begin{flalign*}
|A(C_0,u')\cup\mathcal{V}^{\mathrm{eq}}_{k-1} |+|\mathcal{V}^{\mathrm{change}}_{k-1}|\geq \\
\frac{|A(C_0,u')|}{2}+\frac{|\mathcal{V}^{\mathrm{eq}}_{k-1}|+2|\mathcal{V}^{\mathrm{change}}_{k-1}|}{2}\geq \\
...
% \begin{lemma}[\bf stability]
% Let $C_0\supseteq\dots\supseteq C_k$ be a $(q,k)$-summit for block $B$ and $u$ be a unit such that $D(u)\cap C_k\neq\emptyset$.
% Then either $\vote(u) \geq B$ or $\ww(\mathcal{V}_B) \geq \mathrm{stability}(C)$.
% \end{lemma}

% For a $(q,k)$-summit $C$, we define its stability as

% $$\mathrm{stability}(C) = (2q-\ww(\mathcal{V}))(1-2^{-k}),$$

% The stability of a summit is a measure of credibility given by it to the block $B$.
% The bigger the stability, the more validators would need to equivocate to change the honest nodes' decision about the block, what is formalized as the following lemma: 
...
% $$\mathrm{conf}_V(B, \sigma, q) = \max_{C\in \bar{D}(u), u\in S(V)}\{\conf(C) \mid C \text{ is a summit for } B\}$$

% $$\mathrm{conf}_V(B, \sigma, q) = $$

% \begin{lemma}[\bf block finality]
% Let $V_1$ and $V_2$ be honest nodes with their respective protocol states $\sigma_1,\sigma_2$, and $B_1$ and $B_2$ be competing blocks. 
% Then $\min\Big(\final(B_1,\sigma_1,t_1), \final(B_2,\sigma_2, t_2)\Big)\leq \ww(\mathcal{V}_B)$.
% \end{lemma}


% While it is convenient to define the summit in a 'monotonic' way, so that once the $(q,k)$-summit is found in some protocol state, it remains a valid $(q,k)$-summit for every bigger protocol state, in practice it is more efficient for a validator to not include known equivocators while searching for summits in order to finalize blocks. 
% In consequence, validators using such strategy will not always find maximal summits that exists in their protocol state, but only "honest-so-far summits". 
% To appropriately capture the security guarantees obtained by validators using such strategy, we need an additional definition of confidence for summits produced by honest-so-far validators in protocol state $\sigma$:

% $$\mathrm{conf_{practical}}(C,\sigma) =  \Big(2q-\ww(\mathcal{V})+2\ww\big(E(\sigma)\big)\Big)\big(1-2^{-k}\big),$$

% Note that the confidence computed by the above definition is monotonic in the sense that whenever an additional equivocator $V$ is revealed in the bigger protocol state $\hat{\sigma}>\sigma$, one can reduce the previously found summit $C$ into $\hat{C}$ by removing units produced by $V$.
% Then, $q$ will be possibly reduced by one, but it will be compensated by the $2\ww\big(E(\sigma)\big)$ factor in the equation.

% The following refined version of the Lemma \ref{LemmaConfidence} describes the security guarantees obtained by the validators searching for honest-so-far summits:

% \begin{lemma}[\bf practical confidence]
% Let $C = C_0\cup\dots\cup C_k$ be a $(q,k)$-summit for block $B$ in $\sigma$ consisting solely of honest-so-far validators, and $u$ be a unit such that $D(u)\cap C_k\neq\emptyset$.
% Then either $\vote(u) \geq B$ or $\ww(\mathcal{V}_B) \geq \conf_{practical}(C)$.
% \end{lemma}



% \damian{Notation for validators/units $u,v$, $V$, $\mu, \tau$?}
...
\tcc{Whenever a new unit is created by $V$, its above all maximal units in the local DAG, it is added to the DAG right away.}
\begin{enumerate}
...
% Let $t\geq f$ be a confidence parameter and assume that the number of crashing nodes is $c\leq n-t-\frac{n+t+1}{2}$. If an honest validator reaches a state $\sigma$ such that $\final(\sigma, B, t)$ for some block $B$, then after some point in time, for every state $\sigma'$ reached by any honest validator it holds $\final(\sigma', B, t)$.
% \end{lemma}
...
% \begin{equation}\label{eq:size_H}
%  |H|=n-f-c\geq n-t-\inparen{n-t -  \frac{n+t+1}{2} } =  \frac{n+t+1}{2}.
% \end{equation}
%
...
% $$C_i:=\sigma\cap \{u_{r}^V: V\in H, r\geq r_0+i\} ~~~~~~~\mbox{for }i\geq 0.$$
...
% \begin{lemma}\label{lem:live_technical}
% Let $t\geq f$ be a confidence parameter and assume that the number of crashing nodes is $c\leq n-t-\frac{n+t+1}{2}$. Then for every block height $h$, there exists a block $B_h$ that is eventually finalized by each honest validator.
% \end{lemma}
...
% \begin{remark}
% \damian{Change the language a bit and place this appropriately.}
% We do not quantify what eventually means, because in the asynchronous model, time has no meaning. However, one can easily see that after GST is reached, eventually translates to "$O(1)$ rounds later".
...
%
% However, by making sure that endorsements are reliably broadcast (by any reliable broadcast protocol), we can guarantee that once a unit $u$ is endorsed in a local view of an honest validator $V$, it will also be eventually seen as endorsed by every honest validator.
...
% \begin{theorem}\label{thm:limit_spam}
% Assume that $f<\frac{n}{3}$. Each unit $u$ created by an honest validator in the $N$th round of the protocol justifies at most $O(n N(1 + f_{\mathrm{equiv}}))$ other units, where $f_{\mathrm{equiv}}=|E(D(u))|$.
% \end{theorem}
\noindent 
...
\end{center}
% \damian{I wrote a slightly simpler version than what was written here before. I don't think it matters much how this is done. Feel free to roll it back to the previous version. }
\begin{enumerate}
...
% \item At tick $i>0$,  let $m:=n_v(i)$. Whenever $i$ is divisible by $2^{m+1}$ and $n_v$ did not change in $C$ previous rounds (i.e., it was constant in the interval $[i-C2^{m+1}, i-1]$):
\begin{enumerate}
        \item Let $b_{\mathrm{fin}}$ denote the number of blocks finalized with confidence $t_0$ during the last $C$ rounds,
        \item if $b_{\mathrm{fin}}<C_{\mathrm{fail}}$ then set $n_v(j)=\min(m+1, \nmax)$, for all $j\geq i$,
        \item if $b_{\mathrm{fin}}>C_{\mathrm{succ}}$ then increment $\cntsucc=\cntsucc+1$, otherwise set $\cntsucc=0$.
...
% \damian{I don't think we should write down any arguments for liveness here.}
...
Below we present a refined strategy that still guarantees that no honest validator gets ever stuck because of lack of endorsements of its own, honest units.
%
At the same time, this strategy allows to send much less endorsements.
